...
from models.minigpt4.common.eval_utils import prepare_texts, init_model, eval_parser, computeIoU
from models.minigpt4.conversation.conversation import CONV_VISION_minigptv2

from models.minigpt4.datasets.datasets.mimic_cxr_dataset import evalMIMICDataset, evalDetectMimicDataset
from models.minigpt4.datasets.datasets.knee_x_ray_dataset import evalKneeXrayDataset, evalDetectKneeXrayDataset
from models.minigpt4.datasets.datasets.radvqa_dataset import evalRadVQADataset
from models.minigpt4.datasets.datasets.nlst_dataset import eval_NLST_Dataset
from models.minigpt4.datasets.datasets.rsna_dataset import evalRSNADataset
from models.minigpt4.datasets.datasets.SLAKE_dataset import evalSLAKEDataset
#import cleaning classes
from models.eval_scripts.clean_json import clean_mimic_json, clean_vqa_json, clean_detection_json
from models.eval_scripts.metrics import MIMIC_BERT_Sim, VQA_BERT_Sim, average_iou
...
def process_kneexray_dataset():
    ...
    data = evalKneeXrayDataset(mimic, vis_processor, img_path)
    ...
    file_save_path = os.path.join(save_path,"KneeXray_inference_results_stage3.json")
    with open(file_save_path,'w') as f:
        json.dump(minigpt4_predict, f)
    clean_kneexray_json(file_save_path, file_save_path)

    # csv file path to save the BERT results per each case
    output_csv_path = 'models/metric_results/bert_similarity_scores.csv'

    # in MIMIC_BERT_Sim add the path of the ground_truth then the path of the inference result
    average_similarity = KneeXray_BERT_Sim(eval_file_path, file_save_path, output_csv_path)
    #print the average BERT_Sim
    print("Average BERT Similarity:", average_similarity)


def process_detect_kneexray():
    eval_file_path = cfg.evaluation_datasets_cfg[dataset]["eval_file_path"]
    img_path = cfg.evaluation_datasets_cfg[dataset]["img_path"] 
    batch_size = cfg.evaluation_datasets_cfg[dataset]["batch_size"]
    max_new_tokens = cfg.evaluation_datasets_cfg[dataset]["max_new_tokens"]
    ...
    data = evalDetectKneeXrayDataset(kneexray, vis_processor, img_path)
    eval_dataloader = DataLoader(data, batch_size=batch_size, shuffle=False)
    minigpt4_predict = defaultdict(list)
    resamples = []
    ...
    file_save_path = os.path.join(save_path,"Detect_KneeXray_inference_result.json")
    with open(file_save_path,'w') as f:
        json.dump(minigpt4_predict, f)
    

    csv_pth = os.path.join(save_path,"KneeXray_IoU_results.csv")
    clean_detection_json(file_save_path,file_save_path)
    average_iou(eval_file_path, file_save_path, "to be specified soon", 100, "KneeXray", csv_pth)
...
def process_vqa_dataset():
    ...
    output_csv_path = 'models/BERT_Sim_results/vqa_bert_similarity_scores.csv'
