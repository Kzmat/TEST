...
def modify_args(args):
    ...
    if args.data == 'cifar10':
        ...
        args.num_classes = 4  #四个类别
        args.image_size = 224  # 减小图像尺寸以节省显存
    else:
    ...
    if not hasattr(args, "save_path") or args.save_path is None:
        ...
        # 替换特殊字符
        sanitized_time = re.sub(r'[<>:"/\\|?*]', '-', current_time)
        args.save_path = f"outputs/{args.arch}_{args.evalmode}_{args.data}_{sanitized_time}_{args.num_clients}_{args.num_rounds}_{args.sample_rate}_{args.alpha}"
...
# 添加 --cfg-path 参数
exp_group.add_argument('--cfg-path', default='default.yaml', help="path to configuration file.")
...
data_group.add_argument('--data', metavar='D', default='cifar100',
                        choices=['cifar10', 'cifar100', 'imagenet', 'sst2', 'ag_news','rsna','kneexray'],
